...
export class CrustService {
  ...
  async uploadToCrust(filePath: string, metadata?: any) {
    if (!this.GATEWAY_URL) {
      this.logger.error('CRUST_GATEWAY_URL environment variable is not defined');
    ...
    try {
      ...
      const data = await response.json() as { error?: string; Hash?: string; Name?: string; Size?: string };
