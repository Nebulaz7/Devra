### AI Dataset Verifier: Pre-Encryption Quality Engine 🚀

**Innovation Spotlight**: In our decentralized marketplace, data isn't just listed—it's *vetted* by a hybrid AI powerhouse before minting as an NFT. Enter `main.py`: a blazing-fast FastAPI microservice that dissects uploaded datasets (ZIPs or singles) like a digital forensic expert, scoring them across four axes—**quality**, **completeness**, **consistency**, and **relevance**—while flagging red flags. No more garbage-in-garbage-out; only premium, verifiable datasets hit the blockchain. Built for hackathon velocity, it leverages BERT for text perplexity, ResNet-50 for image confidence, and SentenceTransformer for semantic relevance, all containerized for seamless backend integration.

#### Core Workflow: From Upload to Verdict (Under 10s Latency)
1. **Ingestion & Parsing** 🧠  
   - Accepts ZIP archives or raw files (JSON, CSV, Parquet, Excel, TXT, PNG/JPEG).  
   - Extracts text (recursive JSON traversal, tabular flattening) and images (PIL preprocessing).  
   - Handles edge cases: unsupported formats trigger instant "unsupported" issues.

2. **Bad Data Sentinel** ⚠️  
   - **Tabular Scrutiny**: Flags >30% missing values, >10% duplicates, or Z-score outliers (>3σ) in numerics.  
   - **Text Patrol**: Rejects empty/gibberish (<10% alphanum), too short (<1 char), or novella-length (>500 chars) content.  
   - **Image Audit**: Bans tiny (<50px), mostly-black (>95% pixels), or corrupt visuals.  
   - Output: Structured `Issue` list (e.g., `{file: "data.csv", type: "missing", details: "Col 'price' 45% NaN"}`).

3. **Scoring Symphony** 🎼  
   - **Text Pipeline (BERT-MaskedLM)**: Computes perplexity on top-5 snippets (trunc @512 tokens); maps to quality (100 - avg_perp * 2). Completeness: sample count bonus. Consistency: low perplexity variance.  
   - **Image Pipeline (ResNet-50 ImageNet)**: Top-5 softmax confidence on top-3 images; quality = avg_conf * 100. Completeness: image count scaling.  
   - **Relevance Booster (SentenceTransformer)**: Cosine sim between dataset description and embeddings; scales [-1,1] → [0,100]. *Hack Highlight*: Ties scores to seller's description for topic-fit magic!  
   - **Fusion Logic**: Weighted blend (60% text, 40% image) for multimodal datasets; defaults to dominant modality.

4. **Verdict & Response** ✅  
   - Aggregates into `VerifyResponse`: `{scores: {quality: 85, ...}, status: "VERIFIED" if quality >=50 else "FAILED", issues: [...]}`.  
   - Health endpoint (`/`) for Docker orchestration; runs on CUDA/CPU for demo scalability.

#### Tech Stack & Hackathon Hacks
- **Libs**: FastAPI (API glue), Torch/Transformers (ML brains), Pandas/PyArrow/OpenPyXL (data wrangling), SentenceTransformers (embedding wizardry).  
- **Perf Tweaks**: Limits (e.g., 5 texts, 3 images) for sub-10s inference; eval mode + no_grad() for GPU thrift.  
- **Integration Ready**: Hooks into backend via `POST /verify` (multipart file + optional description). Pre-encryption focus ensures zero leaks—keys handled upstream.  
- **Demo Gold**: Upload a "bad" ZIP (dupe CSVs + blurry PNGs) → Watch issues pile up and scores tank. Swap for clean multimodal data → "VERIFIED" with 90+ scores. Blockchain minting only greenlights the gems!

This verifier isn't just code—it's the trust layer powering our NFT marketplace. Raw datasets in, polished scores out: because in Web3, quality *is* the currency. Fork, deploy, disrupt! 🌟
